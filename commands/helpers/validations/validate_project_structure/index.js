...
export default (project_path) => {
  ...
  const resources_types = ['apis', 'crons', 'shared', 'topics', 'sockets', 'cloud_functions', 'external_topics'];

  const less_allowed_folders_message = 
    `Should contain only folders that their names matches at least one of the following:${
  ...
  fs.readdirSync(project_less_path).forEach(resource_type => {
    ...
    if (
      ...
      throw new ResourceNameInvalidException(`Invalid resource "${resource_type}" on "less" folder.\n${less_allowed_folders_message}`);
    }
  ...
  if (!resources_found) {
    throw new ResourceHandlerNotFoundException(`No resources found on "less" folder. ${less_allowed_folders_message}`);
  }
